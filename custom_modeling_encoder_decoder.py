...
class EncoderDecoderModel(PreTrainedModel):
    ...
    def forward(
        ...
        if not self.split_embedding:
            print('[WARNING] split_embedding is set to False')
        ...
        # Training phase
        if self.split_embedding and self.is_training:
            split_size = encoder_hidden_states.shape[2] // 2
            embed1, embed2 = torch.split(encoder_hidden_states, split_size_or_sections=split_size, dim=-1)
            embed1 = embed1 - embed2 + embed2.detach()
            embed2 = embed2 - embed1 + embed1.detach()
            encoder_hidden_states = torch.cat((embed1, embed2), dim=-1)
            # Re-assign the changed hidden encoder_hidden_states after the split embedding disentangled trick
            # We do so because it might be used later on by transformers
            encoder_outputs['last_hidden_state'] = encoder_hidden_states
            encoder_outputs.last_hidden_state = encoder_hidden_states

        # Inference phase
        if self.split_embedding and not self.is_training:
            split_size = encoder_hidden_states.shape[2] // 2
            embed1, embed2 = torch.split(encoder_hidden_states, split_size_or_sections=split_size, dim=-1)
            embed1 = embed1 * self.embed1
            embed2 = embed2 * self.embed2
            encoder_hidden_states = torch.cat((embed1, embed2), dim=-1)
            # Re-assign the changed hidden encoder_hidden_states after the split embedding disentangled trick
            # We do so because it might be used later on by transformers
            encoder_outputs['last_hidden_state'] = encoder_hidden_states
